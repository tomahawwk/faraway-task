...
const AnimatedLetter: FC<AnimatedLetterType> = ({symbol, index}) => {
  return (
    <span
      key={`symbol-${index}`}
      style={symbol.style}
      dangerouslySetInnerHTML={{__html: symbol.text}}
      className="animated-word__letter"></span>
  );
};
  ...
  return (
    <div className="animated-word">
      <div className="animated-word__wrapper">
        <div className="animated-word__part animated-word__part--top">
          {array.map((symbol, index) => (
            <AnimatedLetter
              symbol={symbol}
              index={index}
              key={`top-${symbol.style.transitionDelay}`}
            />
          ))}
        </div>
        <div className="animated-word__part animated-word__part--bottom">
          {array.map((symbol, index) => (
            <AnimatedLetter
              symbol={symbol}
              index={index}
              key={`bottom-${symbol.style.transitionDelay}`}
            />
          ))}
        </div>
      </div>
